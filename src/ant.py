...
class Ant(Entity):
    def __init__(self, nest):
        ...
        self._create_state("y",0.95)

        self._create_state("food_position",None)
    def do(self):
        """
    ...
    def walk(self):
        ...
        def paper_walk():
            ...
            V = 0# I have no idea what kind of number V should have. Should be incorporated into the state dictionary. I figure let's start with it at 0 and move from there. At 0 the "different realöms thing wont be a part of the model"
            r = 0.1 
            y = self.states["y"]**(r+1)
            self.states["y"] = y
            food_position = self.states["food_position"]
            if y <= 0.000001:
                y = 0
            a = 1000
             #just grabbing a number out of thin air to get started
            b = np.log(2)
            w = 0.2
            psi = 7
            ...
            for i in range(0,2):
                temppos[i] = (
                    (p[i] + V) * np.exp((1-np.exp(-a*y)) * (3-psi*(p[i]+V)))
                    - V
                    + np.exp(-2*a*y+b) * (np.abs(np.sin(w*time))* (food_position[i] - self.nest.position[i]) - (p[i] - self.nest.position[i]))
                )
            next_position = temppos
        ...
        def random_walk():
            ...
            return p + np.random.rand(2)-0.5

        self.next_states["position"] = paper_walk()
